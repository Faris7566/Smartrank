...
function renderTable(){
  ...
  rows.forEach(r=>{
    ...
    tr.innerHTML = `
      <td data-label="Mapel"><input type="text" value="${r.name}" ${isDefault ? "readonly" : ""} data-id="${r.id}" data-key="name" /></td>
  ...
  $$("input[data-key]").forEach(el=>{
    el.oninput = (e)=>{
      ...
      if(key==="name") { row.name = el.value; }
        ...
        meanSpan.textContent = fmt(subjectMean(row));
      }
      saveSession();
    };
  });
...
/* bind buttons */
$("#btnRun").onclick = runCalcAndRender;
$("#btnFill").onclick = ()=>{
...
$("#btnAddRow").onclick = ()=>{ addRow("Mapel Kustom","UMUM",["","","","",""], false); renderTable(); saveSession(); };
$("#btnSave").onclick = ()=> saveLocal();
$("#btnLoad").onclick = ()=> loadLocal();
$("#btnReset").onclick = ()=> { if(confirm("Reset ke daftar mapel default?")){ loadDefaults(); renderTable(); saveSession(); toast("♻️ Reset selesai."); } };
...
$("#fileUpload").addEventListener("change", async (ev)=>{
  ...
  if(!(name.endsWith(".xls") || name.endsWith(".xlsx"))){
    toast("⚠️ Harap unggah file .xls atau .xlsx"); return;
  }
  try{
    ...
    for(let i=0;i<json.length;i++){
      ...
      // find columns by position: assume either header exists or exact order
      // Try to detect header if contains "Nama" or "Mapel"
      if(i===0){
      ...
      // validate category
      const catNorm = CATS.includes(cat) ? cat : (cat.toUpperCase()==="BAHASA" ? "BAHASA" : "UMUM");
    ...
    if(parsed.length===0){ toast("⚠️ File terdeteksi kosong atau format tidak sesuai."); return; }
    // replace rows with parsed (keep category paten if matches)
    rows = []; // reset
    parsed.forEach(([n,c,arr])=>{
      // ensure category is one of CATS; if not, map fallback
    ...
    console.error(err); toast("❌ Gagal baca file XLS.");
  }
});
...
function catLegend(){
  ...
  if(panel) panel.querySelector(".panel-head").appendChild(legendHolder);
}
